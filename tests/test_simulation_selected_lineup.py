...
def test_generate_simulation_selected_lineup_ranks_real_diverse_candidates(engine, stable_slate):
...
def test_recommended_lineup_is_not_just_the_highest_raw_mean_score(engine, stable_slate):
...
def test_select_best_by_simulation_requires_at_least_two_candidates(engine, stable_slate):
    ranked, _, _ = generate_simulation_selected_lineup(
        stable_slate, num_candidates=3, engine=engine
    )
    with pytest.raises(ValueError, match="at least 2"):
        select_best_by_simulation([ranked[0]["lineup"]], stable_slate, engine=engine)


def test_generate_simulation_selected_lineup_is_deterministic_with_a_seed(engine, stable_slate):
    r1, _, _ = generate_simulation_selected_lineup(
        stable_slate, num_candidates=6, seed=42, engine=engine
    )
