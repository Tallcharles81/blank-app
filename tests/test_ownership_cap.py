...
def test_ownership_cap_is_off_by_default(engine):
...
def test_ownership_cap_forces_a_real_lower_ownership_roster(engine):
    players, _, _, _ = _load_player_pool("dk_thu_mon_2026_09_17", "proj_ceiling", engine)
...
def test_ownership_cap_too_tight_raises_value_error(engine):
    players, _, _, _ = _load_player_pool("dk_thu_mon_2026_09_17", "proj_ceiling", engine)
    ownership_values_by_id = {p["player_id"]: p["points"] / (p["salary"] / 1000.0) for p in players}
