...
def generate_opponent_lineups(players, contest_size, concentration=DEFAULT_CONCENTRATION, seed=None):
    """contest_size independently-drawn, salary-cap/roster-feasible opponent
    lineups from the same player pool, weighted toward ownership_proxy via
    ...
    """
    proxy_by_id = ownership_proxy(players)
...
def build_contrarian_lineup(players, lambda_penalty):
    """Maximize (proj_median - lambda_penalty * ownership_proxy) under the
...
def find_contrarian_matching_projection(players, target_projection, tolerance=0.05):
    ...
    within tolerance, picks the one with the LOWEST average ownership_proxy
    ...
    for lam in _LAMBDA_SEARCH_GRID:
        lineup = build_contrarian_lineup(players, lam)
